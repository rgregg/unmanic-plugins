...
"""
    ...
    Modified by:              rgregg - added keep_default_stream option
...
from keep_stream_by_language_custom.lib.ffmpeg import StreamMapper, Probe, Parser

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.keep_stream_by_language_custom")
...
class Settings(PluginSettings):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form_settings = {
            ...
            "keep_default_stream": {
                "description": "If checked, the default/original audio stream will always be kept regardless of its language. "
                               "This is useful for foreign language films where you want to keep the original audio plus your preferred language.",
                "label": "Always keep the default (original) audio stream",
            },
            "keep_commentary":   {
...
class PluginStreamMapper(StreamMapper):
    def __init__(self):
        ...
        self.settings = None
        self.default_audio_indices = set()

    def set_settings(self, settings):
        self.settings = settings

    def set_default_audio_indices(self, probe_streams):
        """Identify audio streams marked as default disposition."""
        self.default_audio_indices = set()
        audio_idx = 0
        for stream in probe_streams:
            if stream.get('codec_type') == 'audio':
                ...
                if disposition.get('default', 0) == 1:
                    self.default_audio_indices.add(audio_idx)
                    logger.debug("Stream index {} (audio:{}) is marked as default".format(stream.get('index'), audio_idx))
                audio_idx += 1
    ...
    def null_streams(self, streams):
        ...
        # If keep_default_stream is on, we'll always have at least one stream to keep
        if self.settings.get_setting('keep_default_stream') and self.default_audio_indices:
            return True
        if (any(l in audio_streams_list for l in alcl) or alcl == ['*'] or audio_streams_list == []):
            return True
    ...
    def same_streams_or_no_work(self, streams, keep_undefined):
        ...
        # If keep_default_stream is on, check if default stream's language is already in the keep list
        # If it is, no extra work needed for default stream; if not, there IS work to do
        if self.settings.get_setting('keep_default_stream') and self.default_audio_indices:
            for idx in self.default_audio_indices:
                audio_count = 0
                for s in streams:
                    if s.get('codec_type') == 'audio':
                        if audio_count == idx:
                            ...
                            try:
                                ...
                                std_lang = lang
                            if std_lang not in alcl and alcl != ['*']:
                                # Default stream language not in keep list — there's work to do
                                logger.debug("Default audio stream language '{}' not in keep list, work needed".format(std_lang))
                                return False
                        audio_count += 1
...
def kept_streams(settings):
    ...
    kd = settings.get_setting('keep_default_stream')
    if not kd:
        kd = settings.settings.get('keep_default_stream')

    return 'kept_streams=audio_langauges={}:subtitle_languages={}:keep_undefined={}:keep_commentary={}:fail_safe={}:keep_default_stream={}'.format(al, sl, ku, kc, fs, kd)
...
def on_library_management_file_test(data):
    ...
    mapper.set_default_audio_indices(probe_streams)
...
def keep_default_audio_streams(mapper, probe_streams, default_audio_indices, kept_audio_indices):
    """Map default audio streams that weren't already kept by language matching."""
    audio_idx = 0
    for stream in probe_streams:
        if stream.get('codec_type') == 'audio':
            if audio_idx in default_audio_indices and audio_idx not in kept_audio_indices:
                lang = stream.get('tags', {}).get('language', 'unknown')
                logger.debug("Keeping default audio stream {} (audio:{}) with language '{}'".format(
                    stream.get('index'), audio_idx, lang))
                mapadder(mapper, audio_idx, 'a')
            audio_idx += 1
...
def on_worker_process(data):
    ...
    keep_default = settings.get_setting('keep_default_stream')
    ...
    if not file_streams_already_kept(settings, data.get('original_file_path')):
        ...
        if mapper.same_streams_or_no_work(probe_streams, keep_undefined_lang_tags):
            ...
            kept_audio_indices = set()
            if not keep_all_audio:
                kept_audio_indices = keep_languages(mapper, 'audio', settings.get_setting('audio_languages'), probe_streams, keep_undefined_lang_tags, keep_commentary)

                # Keep default audio stream regardless of language
                if keep_default and mapper.default_audio_indices:
                    keep_default_audio_streams(mapper, probe_streams, mapper.default_audio_indices, kept_audio_indices)
